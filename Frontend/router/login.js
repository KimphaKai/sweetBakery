...
loginRouter.post('/memberRegister', function (req, res) {
    ...
    if (compareEmail == 0) {
        //可以註冊帳號
        db.query(`INSERT INTO member (memberId, email, userPassword, userName,  userPhone, userBirthday, fb, google) VALUES ('${req.body.memberRegisterEmail}', '${req.body.memberRegisterEmail}','${req.body.memberRegisterPassword}', '','${req.body.memberRegisterPhone}','${req.body.memberRegisterBirthday}','','')`, (error, rows) => {
            if (error) {
                console.log(error);
            }
        })
